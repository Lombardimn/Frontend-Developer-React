...
export function Survey() {
    ...
    const foundSurvey = Array.isArray(mappedSurveys)
        // eslint-disable-next-line react/prop-types
        ? mappedSurveys.find((survey) => survey.idSurvey === parseInt(id))
        : null;
            ...
            <article className="survey-item">
                <section className="survey-header">
                    ...
                    <div className="survey-data">
                        <h2 className="survey-title">{foundSurvey.title}</h2>
                        <p className="survey-description">{foundSurvey.description}</p>
                    </div>
                </section>
                <section className="survey-questions">
                    <h3 className="survey-subtitle">Preguntas</h3>
                    {
                        !foundSurvey.questions
                            ? <p className="survey-subtitle">Sin preguntas</p>
                            : <section>
                                {
                                    foundSurvey.questions.map(question => (
                                        <div key={question.idQuestion} className="survey-box">
                                            <svg
                                                    xmlns="http://www.w3.org/2000/svg"
                                                    width="32"
                                                    height="32"
                                            ...
                                            <ul key={`ul-${question.idQuestion}`}
                                                className="survey-options">
                                                {
                                                    question.options.map(option => (
                                                        <li key={`li-${option.idOption}`} className="survey-option">
                                                            <input 
                                                                type="radio"
                                                            ...
                                                            />
                                                            <p className="survey-option-text">{option.option}</p>
                                                        </li>
                                                    ))
                                                }
                                            </ul>
